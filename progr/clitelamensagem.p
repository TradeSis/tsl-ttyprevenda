...
DEFINE VARIABLE msg AS CHARACTER format "x(20)" EXTENT 20 NO-UNDO.

DEFINE VARIABLE new_lang AS CHARACTER format "x(20)"
...
FORM
  SPACE(2) pergunta FORMAT "x(60)" SPACE(2) SKIP
...
FORM
  SPACE(5) msg[1] SPACE(3) msg[2] SKIP
  WITH FRAME dbox-top OVERLAY NO-LABELS ATTR-SPACE NO-BOX
  WIDTH 60 CENTERED ROW MAXIMUM(6,(SCREEN-LINES - d) / 2) + d
  COLOR messages.

ASSIGN
  d      = 1
  msg[d] = pergunta.
DO WHILE (LENGTH(msg[d]) > 60 OR INDEX(msg[d],"!") > 0) AND d < 20:

  ASSIGN
    i          = MAXIMUM(
                 R-INDEX(SUBSTRING(msg[d],1,60)," "),
                 R-INDEX(SUBSTRING(msg[d],1,60),","))
    i          = (IF i = 0 THEN 60 ELSE i)
    j          = (IF INDEX(msg[d],"!") = 0 THEN 60 ELSE INDEX(msg[d],"!"))
    i          = MINIMUM(i,j)
